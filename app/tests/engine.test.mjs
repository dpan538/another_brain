...
test("the persona carries the four identities, leads with short / plain / never invented, and holds no imagery to lift", () => {
  ...
  for (const never of ["沪", "野人", "夜莺", "番茄", "排队", "三个世界", "暗处", "底片", "三明治", "保温杯", "贝壳", "镜面", "甜筒", "拿铁"]) assert.ok(!PERSONA_PROMPT.includes(never), `imagery in the prompt: ${never}`);
  assert.ok(PERSONA_PROMPT.indexOf("最重要的五条") < 80, "the rules come first");
  assert.ok(!PERSONA_PROMPT.includes("已经为时已晚") && !PERSONA_PROMPT.includes("在上海长大"), "the owner's corrections hold");
