...
const out = join(tmpdir(), `r28ship2-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28ship2"), join(out, "tests/r28ship2"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await cp(join(root, "docs"), join(out, "docs"), { recursive: true });
await cp(join(root, "scripts"), join(out, "scripts"), { recursive: true });
...
const result = spawnSync("node", ["--test", join(out, "tests/r28ship2/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
