...
def main() -> None:
    ...
    if not admission["ok"]:
        write_json(report_dir / "preflight_ledger.json", {"ok": False, "campaign_id": CAMPAIGN_ID, "stop_reason": "source_admission_failed", "admission": admission, **NON_CLAIMS}); return
    device = _resolve_device("mps")
    if POLICY["require_mps"] and device != "mps":
        write_json(report_dir / "preflight_ledger.json", {"ok": False, "campaign_id": CAMPAIGN_ID, "stop_reason": "mps_required_but_unavailable", **NON_CLAIMS}); return
    tokenizer_path = _resolve_tokenizer_path()
    if tokenizer_path is None:
        write_json(report_dir / "preflight_ledger.json", {"ok": False, "campaign_id": CAMPAIGN_ID, "stop_reason": "tokenizer_missing", **NON_CLAIMS}); return
    ...
    if min(len(train), len(dev), len(heldout)) <= context + 1:
        write_json(report_dir / "preflight_ledger.json", {"ok": False, "campaign_id": CAMPAIGN_ID, "stop_reason": "candidate_split_too_small", "token_counts": {"train": len(train), "dev": len(dev), "heldout": len(heldout)}, **NON_CLAIMS}); return
    ...
    ledger = {"ok": True, "campaign_id": CAMPAIGN_ID, "created_at_utc": now_utc(), "train_started": True, "training_ran": True, "selected_device": device, "resume_from": str(args.checkpoint), "token_counts": {"train": len(train), "dev": len(dev), "heldout": len(heldout)}, "baseline": baseline, "admission": admission, "policy": POLICY, "optimizer_tokens": 0, "optimizer_steps": 0, "segments": [], **NON_CLAIMS}; write_json(report_dir / "preflight_ledger.json", ledger)
