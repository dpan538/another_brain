...
const out = join(tmpdir(), `r28ux6-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28ux6"), join(out, "tests/r28ux6"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28ux6/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
