...
const out = join(tmpdir(), `r28ux2-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28ux2"), join(out, "tests/r28ux2"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28ux2/*.mjs")], {
  cwd: root,
  shell: true,
  stdio: "inherit"
});
