...
const APPROVAL_PATH = "training/from_scratch/APPROVE_R25Y_DATA_REGULARIZATION_PILOT.json";
const OUTPUT_DIR = "artifacts/training_os/small_decoder_pilot/r25y/";
const RUN_REPORT_PATH = `${OUTPUT_DIR}r25y_small_decoder_run_report.json`;
const DATASET_REPORT_PATH = `${OUTPUT_DIR}r25y_dataset_report.json`;
const CHECKPOINT_PATH = `${OUTPUT_DIR}r25y_replayable_checkpoint.json`;
const EVAL_REPORT_PATH = `${OUTPUT_DIR}r25y_small_decoder_eval_report.json`;
...
function validateReplayableCheckpoint(checkpoint) {
  ...
  if (checkpoint.run_id !== "r25y_data_regularized_192") failures.push({ code: "checkpoint_run_id_invalid", actual: checkpoint.run_id });
  ...
  if (Number(checkpoint.architecture?.layers) !== 1) failures.push({ code: "checkpoint_must_preserve_one_layer_baseline", actual: checkpoint.architecture?.layers });
...
async function main() {
  ...
  if (config.run_id !== "r25y_data_regularized_192" || config.variant_id !== "r25y_data_regularized_192") {
    failures.push({ code: "r25y_config_variant_mismatch", run_id: config.run_id, variant_id: config.variant_id });
  }
  if (config.phase_4_scaled_training !== false) failures.push({ code: "r25y_config_phase_4_must_be_false" });
  if (Number(config.architecture?.layers) !== 1) failures.push({ code: "r25y_config_must_keep_one_layer_baseline", actual: config.architecture?.layers });
  if (config.architecture?.basis !== "r25s_baseline_data_first") failures.push({ code: "r25y_config_must_be_based_on_r25s", actual: config.architecture?.basis });
  if (!datasetReport?.ok) failures.push({ code: "r25y_dataset_report_missing_or_not_ok" });
  if (!runReport) failures.push({ code: "r25y_run_report_missing", path: RUN_REPORT_PATH });
  if (!checkpoint) failures.push({ code: "r25y_replayable_checkpoint_missing", path: CHECKPOINT_PATH });
  if (approval?.consumed !== true) failures.push({ code: "r25y_approval_not_consumed" });
  if (approval?.allow_weight_commit !== false) failures.push({ code: "r25y_approval_allows_weight_commit" });
  if (approval?.allow_product_model_training !== false) failures.push({ code: "r25y_approval_allows_product_training" });
  if (approval?.allow_long_term_training !== false) failures.push({ code: "r25y_approval_allows_long_term_training" });
  if (approval?.allow_phase_4_scaled_training !== false) failures.push({ code: "r25y_approval_allows_phase_4_scaled_training" });
  if (approval?.allow_release_checkpoint !== false) failures.push({ code: "r25y_approval_allows_release_checkpoint" });

  if (runReport) {
    if (runReport.ok !== true) failures.push({ code: "r25y_run_report_not_ok" });
    if (runReport.run_id !== "r25y_data_regularized_192") failures.push({ code: "r25y_run_id_invalid", actual: runReport.run_id });
    if (runReport.variant_id !== "r25y_data_regularized_192") failures.push({ code: "r25y_variant_id_invalid", actual: runReport.variant_id });
    if (runReport.small_pilot_training_ran !== true) failures.push({ code: "r25y_training_did_not_run" });
    if (runReport.data_regularization_training !== true) failures.push({ code: "r25y_data_regularization_flag_missing" });
    if (Number(runReport.actual_layers) !== 1) failures.push({ code: "r25y_actual_layers_must_be_one", actual: runReport.actual_layers });
    ...
    if (runReport.regularization_knobs?.lower_learning_rate_than_r25s !== true) failures.push({ code: "lower_lr_regularization_not_recorded_active" });
    if (runReport.regularization_knobs?.cap_repeated_targets !== true) failures.push({ code: "target_cap_regularization_not_recorded_active" });
    ...
    for (const path of runReport.artifact_paths || []) {
      if (!String(path).startsWith(OUTPUT_DIR)) failures.push({ code: "artifact_outside_r25y_output_dir", path });
  ...
  if (datasetReport?.balanced_sampling_used !== true) failures.push({ code: "r25y_dataset_did_not_use_balanced_sampling_plan" });
  if (datasetReport?.data_regularization_used !== true) failures.push({ code: "r25y_dataset_did_not_record_data_regularization" });
  ...
  const output = {
    ...
    data_regularization_training: runReport?.data_regularization_training === true,
    backend: runReport?.backend || "unknown",
    actual_layers: runReport?.actual_layers ?? null,
    learning_rate: runReport?.learning_rate ?? null,
    regularization_knobs: runReport?.regularization_knobs || null,
    ...
    artifacts_under_ignored_path: failures.every((failure) => failure.code !== "artifact_outside_r25y_output_dir" && failure.code !== "artifact_not_ignored"),
