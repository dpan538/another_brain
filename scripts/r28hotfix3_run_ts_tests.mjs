...
const out = join(tmpdir(), `r28hotfix3-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28hotfix3"), join(out, "tests/r28hotfix3"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await mkdir(join(out, "scripts"), { recursive: true });
await cp(join(root, "scripts/r28hotfix3_q4_asset_path_audit.py"), join(out, "scripts/r28hotfix3_q4_asset_path_audit.py"));
await cp(join(root, "scripts/r28hotfix3_static_asset_fetch_smoke.py"), join(out, "scripts/r28hotfix3_static_asset_fetch_smoke.py"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28hotfix3/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
