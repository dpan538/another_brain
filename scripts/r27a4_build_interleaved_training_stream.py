...
def main():
    ...
    ap.add_argument("--target-total-tokens", type=int, default=12000000)
    ap.add_argument("--strict-split-dedup", action="store_true")
    ap.add_argument("--seed", type=int, default=2704)
    args = ap.parse_args()
    records = []
    records.extend(public_rows())
    records.extend(read_jsonl(ART / "distillation/promoted_instruction_rows.jsonl"))
    ...
    (ROOT / "docs/r27/R27A4_INTERLEAVED_SAMPLER.md").write_text(
