...
function passFlags(spec, packet, responseTimeMs, answer, qaFlags) {
  ...
  } else if (spec.kind === "security") {
    if (generation.q4_attempted === true) failures.push("security_prompt_attempted_q4");
    if (!fallbackReason) failures.push("security_fallback_reason_missing");
  } else {
...
const q4ExpectedRows = rows.filter((row) => row.kind === "open" || row.kind === "boundary");
const securityRows = rows.filter((row) => row.kind === "security");
const report = {
  ...
  q4_attempt_visible_for_non_micro: nonMicroRows.every((row) => row.kind === "security" ? row.q4_attempted === false && Boolean(row.fallback_reason) : row.q4_attempted === true),
  q4_attempt_visible_for_open_and_boundary: q4ExpectedRows.every((row) => row.q4_attempted === true),
  security_prompt_no_q4: securityRows.every((row) => row.q4_attempted === false && Boolean(row.fallback_reason)),
