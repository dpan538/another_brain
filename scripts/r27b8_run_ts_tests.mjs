...
const out = join(tmpdir(), `r27b8-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r27b8"), join(out, "tests/r27b8"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r27b8/*.mjs")], {
  cwd: root,
  shell: true,
  stdio: "inherit"
});
