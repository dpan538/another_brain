...
def main():
    ...
    write_jsonl(ROOT / "artifacts/r27a5/curriculum/reasoning_symbolic.jsonl", rows)
    report = {"ok": len(rows) >= args.target_rows, "rows": len(rows)}
    (ROOT / "artifacts/r27a5/reports").mkdir(parents=True, exist_ok=True)
    (ROOT / "artifacts/r27a5/reports/reasoning_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
