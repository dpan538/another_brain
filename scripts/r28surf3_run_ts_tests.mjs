...
const out = join(tmpdir(), `r28surf3-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28surf3"), join(out, "tests/r28surf3"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await mkdir(join(out, "data/training_registry"), { recursive: true });
await cp(join(root, "data/training_registry/r28surf3_surface_profile.json"), join(out, "data/training_registry/r28surf3_surface_profile.json"), { recursive: true });

const result = spawnSync("node", ["--test", join(out, "tests/r28surf3/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
