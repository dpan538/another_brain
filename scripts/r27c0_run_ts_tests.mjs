...
const out = join(tmpdir(), `r27c0-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r27c0"), join(out, "tests/r27c0"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r27c0/*.mjs")], {
  cwd: root,
  shell: true,
  stdio: "inherit"
});
