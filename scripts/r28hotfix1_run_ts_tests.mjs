...
const out = join(tmpdir(), `r28hotfix1-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28hotfix1"), join(out, "tests/r28hotfix1"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await mkdir(join(out, "scripts"), { recursive: true });
await cp(join(root, "scripts/check_vercel_static_build.mjs"), join(out, "scripts/check_vercel_static_build.mjs"));
await cp(join(root, "vercel.json"), join(out, "vercel.json"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28hotfix1/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
