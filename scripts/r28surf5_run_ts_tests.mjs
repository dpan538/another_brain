...
const out = join(tmpdir(), `r28surf5-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28surf5"), join(out, "tests/r28surf5"));
...
const result = spawnSync("node", ["--test", join(out, "tests/r28surf5/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
