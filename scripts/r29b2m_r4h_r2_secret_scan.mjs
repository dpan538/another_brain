...
const trackedText = tracked.filter((path) => TEXT_EXTENSIONS.has(extname(path).toLowerCase()));
...
for (const path of trackedText) {
  try {
    ...
    // A file that cannot be decoded as text is outside the text-secret contract.
  }
}

let artifactSecretMatches = 0;
let authorizationValueMatches = 0;
for (const path of await walk(ARTIFACT_ROOT)) {
  ...
  try {
    ...
    // Ignored binary artifacts are not part of this live text log scan.
  }
}
...
const report = {
  ...
  scanned_tracked_text_files: trackedText.length,
  scanned_artifact_files: (await walk(ARTIFACT_ROOT)).length,
