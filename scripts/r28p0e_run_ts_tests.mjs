...
const out = join(tmpdir(), `r28p0e-ts-tests-${process.pid}`);
...
await copyTsAsMjs(join(root, "tests/r28p0e"), join(out, "tests/r28p0e"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await mkdir(join(out, "scripts"), { recursive: true });
await cp(join(root, "scripts/r28p0e_browser_q4_answer_smoke.mjs"), join(out, "scripts/r28p0e_browser_q4_answer_smoke.mjs"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28p0e/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
