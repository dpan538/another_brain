...
def main():
    ...
    write_jsonl(ROOT / "artifacts/r27a6/curriculum/rag_evidence.jsonl", rows)
    report = {"ok": len(rows) >= args.target_rows, "rows": len(rows)}
    (ROOT / "artifacts/r27a6/reports").mkdir(parents=True, exist_ok=True)
    (ROOT / "artifacts/r27a6/reports/rag_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
