...
const REPORT_PATH = path.join(OUT_DIR, "r25ak_validation_report.json");
const FILES = {
  train: "training/llm_corpus/r25ak_repo_derived_train.jsonl",
  dev: "training/llm_corpus/r25ak_repo_derived_dev.jsonl",
  heldout: "training/llm_corpus/r25ak_repo_derived_heldout.jsonl"
};
const EXPECTED_COUNTS = { train: 256, dev: 32, heldout: 32 };
...
for (const [split, relativePath] of Object.entries(FILES)) {
  ...
  if (!fs.existsSync(full)) {
    failures.push({ code: "missing_r25ak_corpus_file", file: relativePath });
    continue;
  }
...
const languageCounts = countBy(rows.filter((row) => !row.__parse_error), (row) => row.language || "missing");
const transformationCounts = countBy(rows.filter((row) => !row.__parse_error), (row) => row.transformation_type || "missing");
const personalTargetCounts = countBy(rows.flatMap((row) => row.personal_color_targets || []), (target) => target);
const sourceCategoryCounts = countBy(rows.filter((row) => !row.__parse_error), (row) => row.source_category || "missing");
...
for (const row of rows) {
  ...
  if (row.provenance?.promotion_phase !== "R25AK") failures.push({ code: "promotion_phase_not_r25ak", ...loc });
...
const splitCounts = countBy(rows.filter((row) => !row.__parse_error), (row) => row.split || "missing");
const totalRows = rows.filter((row) => !row.__parse_error).length || 1;
const zhShare = (languageCounts.zh || 0) / totalRows;
const enShare = (languageCounts.en || 0) / totalRows;
if (zhShare < 0.7) failures.push({ code: "zh_share_below_target", zhShare });
if (enShare > 0.1) failures.push({ code: "en_share_above_cap", enShare });
...
const report = {
  report_id: "r25ak_validation_report",
  ok: failures.length === 0,
  promoted_total: totalRows,
