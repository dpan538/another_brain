...
DOC = ROOT / "docs/r27/R27A6_DATA_EXPANSION_AND_MIX_CORRECTION.md"
DEFAULT_SOURCES = "baai_industry_corpus,baai_industry_corpus2,fineweb_2,fineweb,wikipedia_zh,oasst1,baai_coig,baai_coig_pc,baai_industry_instruction,coig_cqia,infinity_instruct"
...
def registry_dataset_from_decision(decision):
    return {
        ...
        "decision_scope": "R27A6 engineering campaign only, not product training, not phase_4, not release",
        "decision_reason": decision["decision_reason"],
        "raw_artifact_path": f"artifacts/r27a6/raw_public_samples/{decision['dataset_id']}/",
        "processed_artifact_path": f"artifacts/r27a6/clean_public_samples/{decision['dataset_id']}/clean.jsonl",
...
def main():
    ...
    ap.add_argument("--max-total-raw-mb", type=int, default=1500)
    ap.add_argument("--max-rows-per-source", type=int, default=100000)
    ap.add_argument("--max-bytes-per-source", type=int, default=400000000)
    args = ap.parse_args()
    if not args.execute:
        args.dry_run = True
    total_bytes = 0
    ...
    for dataset_id in parse_sources(args.sources):
        ...
        decision["decision_scope"] = "R27A6 engineering campaign only, not product training, not phase_4, not release"
    ...
    write_json(DECISIONS, {"generated_at_utc": now_utc(), "r27a6": True, "decisions": decisions})
    ...
    registry["phase"] = "R27A6"
    registry["registry_id"] = "r27a6_public_corpus_registry"
    registry.setdefault("rules", {})
    registry["rules"]["raw_artifacts_root"] = "artifacts/r27a6/raw_public_samples/"
    registry["rules"]["processed_artifacts_root"] = "artifacts/r27a6/clean_public_samples/"
    ...
    DOC.write_text("# R27A6 Data Expansion And Mix Correction\n\n" + "\n".join(f"- `{m['dataset_id']}`: `{m['status']}`, rows `{m['downloaded_rows']}`, bytes `{m['downloaded_bytes']}`." for m in manifests) + "\n\nRaw and cleaned public text remains ignored under `artifacts/r27a6/`. Sources that cannot be safely admitted are recorded as blockers instead of bypassed.\n", encoding="utf-8")
