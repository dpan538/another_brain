...
const out = join(tmpdir(), `r28hotfix0-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28hotfix0"), join(out, "tests/r28hotfix0"));
...
const result = spawnSync("node", ["--test", join(out, "tests/r28hotfix0/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
