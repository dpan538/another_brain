...
const out = join(tmpdir(), `r28hotfix4-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28hotfix4"), join(out, "tests/r28hotfix4"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await mkdir(join(out, "scripts"), { recursive: true });
await cp(join(root, "scripts/r28hotfix4_open_question_pipeline_audit.py"), join(out, "scripts/r28hotfix4_open_question_pipeline_audit.py"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28hotfix4/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
