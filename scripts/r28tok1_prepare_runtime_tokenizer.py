...
from r28tok1_discover_exact_tokenizer import discover_exact_tokenizer  # noqa: E402
...
def write_runtime_tokenizer_json(path: Path, data: dict[str, Any]) -> None:
    path.parent.mkdir(parents=True, exist_ok=True)
    path.write_text(json.dumps(data, ensure_ascii=False, indent=2, sort_keys=True).replace("/", "\\/") + "\n", encoding="utf-8")
...
def strip_runtime_tokenizer(source: Path) -> dict[str, Any]:
    ...
    return {
        "schema_version": "r28tok1.exact_runtime_tokenizer.v1",
        ...
        "source_lineage": {
            ...
            "source_tokenizer": "r27a4_model_lab_bpe_16000_or_equivalent_a_line_tokenizer",
...
def write_checksums() -> dict[str, Any]:
    ...
    payload = {
        "schema_version": "r28tok1.checksums.v1",
        "same_origin_only": True,
        "file_count": len(files),
        "total_bytes": sum(path.stat().st_size for path in files),
        "files": [{"path": web_rel(path), "bytes": path.stat().st_size, "sha256": sha256_file(path)} for path in files],
    }
...
def update_asset_manifest() -> dict[str, Any]:
    ...
    model_assets.extend(asset_entry(WEB_ROOT / item["path"], "q4_shard", index=int(item["index"])) for item in quant.get("shards", []))
    tokenizer_assets = [
    ...
    manifest.update({
        "runtime_version": "r28tok1-exact-tokenizer-runtime-v1",
        "prelaunch_stage": "r28tok1",
...
def update_runtime_mode() -> dict[str, Any]:
    ...
    runtime.update({
        ...
        "candidate_warning": "R28TOK1 admits an exact runtime tokenizer for static q4 compatibility only; this is not product, browser, or release admission.",
        "non_product_warning": "R28TOK1 static q4 runtime uses an exact runtime tokenizer but remains experimental/non-product.",
...
def prepare_runtime_tokenizer() -> dict[str, Any]:
    discovery = discover_exact_tokenizer()
    if not discovery["ok"]:
        return {"ok": False, "exact_tokenizer_found": False, "blocker": discovery.get("blocker"), "discovery": discovery}
    source = Path(discovery["source_path"])
    runtime_tokenizer = strip_runtime_tokenizer(source) if source != RUNTIME_TOKENIZER else read_json(source)
    runtime_tokenizer["schema_version"] = "r28tok1.exact_runtime_tokenizer.v1"
...
def main() -> int:
    report = prepare_runtime_tokenizer()
