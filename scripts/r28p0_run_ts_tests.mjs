...
const out = join(tmpdir(), `r28p0-ts-tests-${process.pid}`);
...
await copyTsAsMjs(join(root, "tests/r28p0"), join(out, "tests/r28p0"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28p0/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
