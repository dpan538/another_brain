...
const out = join(tmpdir(), `r28rt1-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28rt1"), join(out, "tests/r28rt1"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28rt1/*.mjs")], {
  cwd: root,
  shell: true,
  stdio: "inherit"
});
