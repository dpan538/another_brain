...
const out = join(tmpdir(), `r28ship0-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28ship0"), join(out, "tests/r28ship0"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await mkdir(join(out, "scripts"), { recursive: true });
await cp(join(root, "scripts/r28ship0_branch_feature_inventory.py"), join(out, "scripts/r28ship0_branch_feature_inventory.py"));
await cp(join(root, "scripts/r28ship0_product_mount_smoke.py"), join(out, "scripts/r28ship0_product_mount_smoke.py"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28ship0/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
