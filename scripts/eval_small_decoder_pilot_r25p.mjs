...
const APPROVAL_PATH = "training/from_scratch/APPROVE_R25P_SECOND_SMALL_PILOT.json";
const OUTPUT_DIR = "artifacts/training_os/small_decoder_pilot/r25p/";
const RUN_REPORT_PATH = `${OUTPUT_DIR}r25p_small_decoder_run_report.json`;
const DATASET_REPORT_PATH = `${OUTPUT_DIR}r25p_dataset_report.json`;
const CHECKPOINT_PATH = `${OUTPUT_DIR}r25p_replayable_checkpoint.json`;
const EVAL_REPORT_PATH = `${OUTPUT_DIR}r25p_small_decoder_eval_report.json`;
...
function validateReplayableCheckpoint(checkpoint) {
  ...
  if (checkpoint.run_id !== "r25p_more_sequences_128") failures.push({ code: "checkpoint_run_id_invalid", actual: checkpoint.run_id });
...
async function main() {
  ...
  if (config.run_id !== "r25p_more_sequences_128" || config.variant_id !== "r25p_more_sequences_128") {
    failures.push({ code: "r25p_config_variant_mismatch", run_id: config.run_id, variant_id: config.variant_id });
  }
  if (!datasetReport?.ok) failures.push({ code: "r25p_dataset_report_missing_or_not_ok" });
  if (!runReport) failures.push({ code: "r25p_run_report_missing", path: RUN_REPORT_PATH });
  if (!checkpoint) failures.push({ code: "r25p_replayable_checkpoint_missing", path: CHECKPOINT_PATH });
  if (approval?.consumed !== true) failures.push({ code: "r25p_approval_not_consumed" });
  if (approval?.allow_weight_commit !== false) failures.push({ code: "r25p_approval_allows_weight_commit" });
  if (approval?.allow_product_model_training !== false) failures.push({ code: "r25p_approval_allows_product_training" });
  if (approval?.allow_long_term_training !== false) failures.push({ code: "r25p_approval_allows_long_term_training" });
  if (approval?.allow_release_checkpoint !== false) failures.push({ code: "r25p_approval_allows_release_checkpoint" });

  if (runReport) {
    if (runReport.ok !== true) failures.push({ code: "r25p_run_report_not_ok" });
    if (runReport.run_id !== "r25p_more_sequences_128") failures.push({ code: "r25p_run_id_invalid", actual: runReport.run_id });
    if (runReport.variant_id !== "r25p_more_sequences_128") failures.push({ code: "r25p_variant_id_invalid", actual: runReport.variant_id });
    if (runReport.small_pilot_training_ran !== true) failures.push({ code: "r25p_training_did_not_run" });
    ...
    for (const path of runReport.artifact_paths || []) {
      if (!String(path).startsWith(OUTPUT_DIR)) failures.push({ code: "artifact_outside_r25p_output_dir", path });
  ...
  const output = {
    ...
    artifacts_under_ignored_path: failures.every((failure) => failure.code !== "artifact_outside_r25p_output_dir" && failure.code !== "artifact_not_ignored"),
