...
const out = join(tmpdir(), `r28ux4-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28ux4"), join(out, "tests/r28ux4"));
...
const result = spawnSync("node", ["--test", join(out, "tests/r28ux4/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
