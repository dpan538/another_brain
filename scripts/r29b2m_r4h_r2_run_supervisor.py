#!/usr/bin/env python3
"""Blocking foreground supervisor for the bounded R29B2M-R4H-R2 live experiment."""
...
CAMPAIGN_ID = "r29b2m_r4h_r2_grounded_attention_style_v2"
...
class Supervisor:
    def __init__(self, repo: Path, artifact_root: Path, phase: str) -> None:
        ...
        self.state: dict[str, Any] = {
            ...
            "maximum_total_requests": 70,
            "maximum_estimated_cost_cny": 2.0,
            "concurrency": 1,
            "old_R4H_R1_terminal": "BLOCKED_HYBRID_VALUE",
    ...
    def run(self) -> int:
        if not self.state["key_present"]:
            self.transition("BLOCKED_HYBRID_V2_CONFIGURATION")
            atomic_json(self.heartbeat_path, {**self.state, "heartbeat_at": utc_now(), "process_active": False})
            return 2
        self.artifact_root.mkdir(parents=True, exist_ok=True)
        heartbeat = threading.Thread(target=self.heartbeat, name="r4h-r2-heartbeat", daemon=True)
        heartbeat.start()
        self.transition("LIVE_SMOKE_V2" if self.phase == "smoke" else "PRIMARY_PAIRED_AB_V2")
        command = [
            "node",
        ...
        self.state["child_command"] = [
            "node",
            "--experimental-strip-types",
            "r29b2m_r4h_r2_live_experiment.mjs",
            "--phase",
            self.phase,
            "--artifact-root",
            "artifacts/r29b2m_r4h_r2",
        ]
        ...
        if self.interrupted:
            ...
            self.transition("SMOKE_V2_PASSED_AWAITING_PAIRED" if self.phase == "smoke" else "PAIRED_V2_COMPLETE_AWAITING_BLIND_REVIEW")
            result = 0
        self.stop_event.set()
...
def main() -> int:
    ...
    parser.add_argument("--artifact-root", type=Path, default=Path("artifacts/r29b2m_r4h_r2"))
    parser.add_argument("--phase", choices=["smoke", "paired"], required=True)
