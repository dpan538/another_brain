...
const out = join(tmpdir(), `r28surf4-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28surf4"), join(out, "tests/r28surf4"));
...
const result = spawnSync("node", ["--test", join(out, "tests/r28surf4/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
