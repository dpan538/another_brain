...
async function main() {
  ...
  const r26eApproval = await readJsonIfPresent("training/from_scratch/APPROVE_R26E_PROMOTE_FIRST50_USER_ANSWERS.json");
  const r26ePromotionReport = await readJsonIfPresent("artifacts/training_os/user_answer_intake/r26e/r26e_promotion_report.json");
  const r26eValidationReport = await readJsonIfPresent("artifacts/training_os/user_answer_intake/r26e/r26e_validation_report.json");
  const r26eCoverageReport = await readJsonIfPresent("artifacts/training_os/user_answer_intake/r26e/r26e_user_answered_coverage.json");
  const r26fTemplate = await readJsonIfPresent("training/from_scratch/APPROVE_R26F_REPLACEMENT_51_100_INTAKE.template.json");
  const r26gApproval = await readJsonIfPresent("training/from_scratch/APPROVE_R26G_FIX_AND_INTAKE_USER_ANSWERS.json");
  const r26gMetadataFixReport = await readJsonIfPresent("artifacts/training_os/user_answer_intake/r26g/r26g_r26e_metadata_fix_report.json");
  const r26gValidationReport = await readJsonIfPresent("artifacts/training_os/user_answer_intake/r26g/r26g_validation_report.json");
  const r26gCoverageReport = await readJsonIfPresent("artifacts/training_os/user_answer_intake/r26g/r26g_user_answered_coverage.json");
  const r26hTemplate = await readJsonIfPresent("training/from_scratch/APPROVE_R26H_USER_ANSWER_CORPUS_READINESS.template.json");
  const r26hUserAnswerReadinessReport = await readJsonIfPresent("artifacts/training_os/user_answer_readiness/r26h/r26h_user_answer_corpus_readiness.json");
  const r26hTokenizerReadinessReport = await readJsonIfPresent("artifacts/training_os/tokenizer_dryrun/r26h/r26h_tokenizer_readiness_report.json");
  const r26iTrainingEntryPlan = await readJsonIfPresent("artifacts/training_os/user_answer_readiness/r26h/r26i_training_entry_plan.json");
  const r26hTrainingEntryDecision = await readJsonIfPresent("artifacts/training_os/user_answer_readiness/r26h/r26h_training_entry_decision.json");
  const r26iTemplate = await readJsonIfPresent("training/from_scratch/APPROVE_R26I_ANSWER_AS_USER_MICROCYCLE.template.json");
  const r27aRelationEvidenceIndex = await readJsonIfPresent("training/current/relation_evidence_index.r27a.json");
  const r27aValueProfile = await readJsonIfPresent("training/current/value_aesthetic_profile.r27a.json");
  const r27aTeacherPolicy = await readJsonIfPresent("training/current/teacher_distillation_policy.r27a.json");
  const r27aTeacherOutputIntake = await readJsonIfPresent("artifacts/training_os/r27a_architecture/r27a_teacher_output_intake_report.json");
  const r27aP0Eval = await readJsonIfPresent("artifacts/training_os/r27a_architecture/r27a_p0_reasoning_rag_value_eval.json");
  const r27bTemplate = await readJsonIfPresent("training/from_scratch/APPROVE_R27B_P0_DISTILLED_ANSWER_AS_USER_MICROCYCLE.template.json");
  ...
  const r26ePromotionOk = Boolean(
    r26dIntakeOk &&
    r26ePromotionReport?.ok === true &&
    r26eValidationReport?.ok === true &&
    r26eCoverageReport?.ok === true &&
    r26eApproval?.consumed === true &&
    r26eApproval?.allow_additional_runs === false &&
    r26eApproval?.allow_training === false &&
    r26eApproval?.allow_tokenizer_dry_run === false &&
    r26eApproval?.allow_phase_4_scaled_training === false &&
    r26fTemplate?.approved === false &&
    r26fTemplate?.allow_training === false
  );
  const r26gCompleteOk = Boolean(
    r26ePromotionOk &&
    r26gApproval?.consumed === true &&
    r26gApproval?.allow_additional_runs === false &&
    r26gApproval?.allow_training === false &&
    r26gApproval?.allow_tokenizer_dry_run === false &&
    r26gApproval?.allow_small_pilot_training === false &&
    r26gApproval?.allow_phase_4_scaled_training === false &&
    r26gMetadataFixReport?.ok === true &&
    r26gValidationReport?.ok === true &&
    r26gCoverageReport?.ok === true &&
    r26hTemplate?.approved === false &&
    r26hTemplate?.allow_training === false &&
    activeTrainingApprovalCount === 0 &&
    activeTokenizerDryRunApprovalCount === 0 &&
    activePhase4TrainingApprovalCount === 0
  );
  const r26hReadyOk = Boolean(
    r26gCompleteOk &&
    r26hUserAnswerReadinessReport?.ok === true &&
    r26hTokenizerReadinessReport?.recommendation === "tokenizer_ready_for_r26i" &&
    r26iTrainingEntryPlan?.ok === true &&
    r26hTrainingEntryDecision?.decision === "ready_for_r26i_answer_as_user_microcycle" &&
    r26hTrainingEntryDecision?.training_approved_now === false &&
    r26hTrainingEntryDecision?.phase4_approved === false &&
    r26iTemplate?.approved === false &&
    r26iTemplate?.allow_training === false &&
    r26iTemplate?.allow_small_pilot_training === false &&
    r26iTemplate?.allow_phase_4_scaled_training === false &&
    activeTrainingApprovalCount === 0 &&
    activePhase4TrainingApprovalCount === 0
  );
  const r27aArchitectureOk = Boolean(
    r26hReadyOk &&
    r27aRelationEvidenceIndex?.ok === true &&
    r27aValueProfile?.ok === true &&
    r27aTeacherPolicy?.does_not_call_teacher === true &&
    r27aTeacherPolicy?.training_allowed === false &&
    r27aTeacherOutputIntake?.ok === true &&
    r27aP0Eval?.ok === true &&
    r27bTemplate?.approved === false &&
    r27bTemplate?.allow_training === false &&
    r27bTemplate?.allow_small_pilot_training === false &&
    r27bTemplate?.allow_phase_4_scaled_training === false &&
    activeTrainingApprovalCount === 0 &&
    activeTokenizerDryRunApprovalCount === 0 &&
    activePhase4TrainingApprovalCount === 0
  );
  ...
  const report = {
    ...
    r26e_user_answer_promotion_status: r26ePromotionOk ? "promoted_reviewed_first50_user_answer_subset" : "not_complete",
    r26e_promoted_rows_total: r26ePromotionReport?.promoted_total || 0,
    r26e_user_answered_share: r26eCoverageReport?.current_user_answered_share || 0,
    r26f_template_status: r26fTemplate?.approved === false ? "inert_template_present" : "missing_or_needs_review",
    r26g_metadata_fix_status: r26gMetadataFixReport?.ok ? "passed_response_obligation_metadata_fix" : "not_complete",
    r26g_replacement_intake_status: r26gCompleteOk ? "passed_replacement_51_100_intake_and_promotion" : r26gCoverageReport?.ok ? "coverage_report_present_needs_history_gate" : "not_complete",
    r26g_user_answered_promoted_rows_total: r26gCoverageReport?.replacement_51_100_promoted_count || 0,
    r26g_combined_user_answered_rows: r26gCoverageReport?.combined_user_answered_corpus_count_after_r26g || 0,
    r26h_template_status: r26hTemplate?.approved === false ? "inert_template_present" : "missing_or_needs_review",
    r26h_user_answer_readiness_status: r26hUserAnswerReadinessReport?.ok ? "passed_user_answer_corpus_readiness" : "not_complete",
    r26h_tokenizer_readiness_status: r26hTokenizerReadinessReport?.recommendation || "not_complete",
    r26i_training_entry_plan_status: r26iTrainingEntryPlan?.ok ? "frozen_training_entry_plan" : "not_complete",
    r26h_training_entry_decision: r26hTrainingEntryDecision?.decision || "not_complete",
    r26i_template_status: r26iTemplate?.approved === false ? "inert_template_present" : "missing_or_needs_review",
    r27a_reasoning_plan_status: await readJsonIfPresent("training/current/reasoning_plan.schema.json") ? "schema_present_runtime_trace_hooked" : "not_complete",
    r27a_rag_evidence_status: await readJsonIfPresent("training/current/evidence_packet.schema.json") ? "schema_present_runtime_trace_hooked" : "not_complete",
    r27a_relation_evidence_index_status: r27aRelationEvidenceIndex?.ok ? "built_refs_only_index" : "not_complete",
    r27a_value_profile_status: r27aValueProfile?.ok ? "built_aggregate_value_profile" : "not_complete",
    r27a_teacher_probe_status: r27aTeacherOutputIntake?.ok ? "teacher_probe_scaffold_ready_no_outputs_required" : "not_complete",
    r27a_distillation_policy_status: r27aTeacherPolicy?.does_not_call_teacher === true ? "policy_present_no_teacher_calls" : "not_complete",
    r27a_p0_eval_status: r27aP0Eval?.ok ? "passed_p0_reasoning_rag_value_eval" : "not_complete",
    r27a_training_impact: "no_training",
    r27b_template_status: r27bTemplate?.approved === false ? "inert_template_present" : "missing_or_needs_review",
