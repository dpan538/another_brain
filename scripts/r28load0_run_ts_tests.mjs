...
const out = join(tmpdir(), `r28load0-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28load0"), join(out, "tests/r28load0"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28load0/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
