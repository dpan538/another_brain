...
const OUTPUT_PATH = "artifacts/training_os/small_decoder_pilot/r25ac/r25ac_chinese_personal_breakdown.json";
const CHECKPOINT_PATH = "artifacts/training_os/small_decoder_pilot/r25ac/r25ac_replayable_checkpoint.json";
const HELDOUT_SEQUENCES_PATH = "artifacts/training_os/small_decoder_pilot/r25ac/r25ac_heldout_sequences.json";
const DATASET_REPORT_PATH = "artifacts/training_os/small_decoder_pilot/r25ac/r25ac_dataset_report.json";
const RUN_REPORT_PATH = "artifacts/training_os/small_decoder_pilot/r25ac/r25ac_small_decoder_run_report.json";
...
async function main() {
  ...
  if (missing.length) {
    const report = {
      ok: true,
      skipped: true,
      reason: "ignored_r25ac_artifacts_missing",
      missing,
      training_ran: false,
      product_model: false,
      release_checkpoint: false,
      phase_4_scaled_training: false,
      notes: ["R25AC breakdown is skipped because local ignored replay artifacts are absent."]
    };
  ...
  if (config.run_id !== "r25ac_chinese_personal_microcycle_256") failures.push({ code: "config_run_id_mismatch", actual: config.run_id });
  if (approval.consumed !== true || approval.allow_additional_runs !== false) failures.push({ code: "approval_must_be_consumed_inert" });
  if (runReport.small_pilot_training_ran !== true || runReport.chinese_personal_microcycle !== true) failures.push({ code: "r25ac_run_report_not_chinese_personal_training" });
  ...
  for (const sequence of heldoutDataset.sequences || []) {
    ...
    for (const target of sequence.personal_targets || ["none"]) {
      addToBucket(getBucket(byPersonalTarget, target), lossRow, stats);
    }
    for (const tag of sequence.policy_tags || ["unknown"]) {
  ...
  const report = {
    ...
    training_ran: false,
    chinese_personal_microcycle_ran_once: runReport.small_pilot_training_ran === true,
    ...
    target_language_mix: config.language_mix_target,
    ...
    phase_4_scaled_training: false,
    notes: [
      "R25AC breakdown reads only the existing ignored R25AC checkpoint/sequences and R25L-derived metadata.",
      "No training is performed by this evaluator.",
      "Per-bucket losses are weighted by next-token pair count.",
      "Personal target coverage is structural label coverage, not private memory."
    ],
    failures
  };
  await writeJson(OUTPUT_PATH, report);
