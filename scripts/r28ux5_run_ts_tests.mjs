...
const out = join(tmpdir(), `r28ux5-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28ux5"), join(out, "tests/r28ux5"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28ux5/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
