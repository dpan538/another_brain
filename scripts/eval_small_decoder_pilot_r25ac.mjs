...
const CONFIG_PATH = "training/from_scratch/small_decoder_pilot_run_config.r25ac.json";
const APPROVAL_PATH = "training/from_scratch/APPROVE_R25AC_CHINESE_PERSONAL_MICROCYCLE.json";
const OUTPUT_DIR = "artifacts/training_os/small_decoder_pilot/r25ac/";
const RUN_REPORT_PATH = `${OUTPUT_DIR}r25ac_small_decoder_run_report.json`;
const DATASET_REPORT_PATH = `${OUTPUT_DIR}r25ac_dataset_report.json`;
const CHECKPOINT_PATH = `${OUTPUT_DIR}r25ac_replayable_checkpoint.json`;
const EVAL_REPORT_PATH = `${OUTPUT_DIR}r25ac_small_decoder_eval_report.json`;
...
function validateReplayableCheckpoint(checkpoint) {
  ...
  if (checkpoint.run_id !== "r25ac_chinese_personal_microcycle_256") failures.push({ code: "checkpoint_run_id_invalid", actual: checkpoint.run_id });
...
async function main() {
  ...
  if (config.run_id !== "r25ac_chinese_personal_microcycle_256" || config.variant_id !== "r25ac_chinese_personal_microcycle_256") {
    failures.push({ code: "r25ac_config_variant_mismatch", run_id: config.run_id, variant_id: config.variant_id });
  }
  if (config.phase_4_scaled_training !== false) failures.push({ code: "r25ac_config_phase_4_must_be_false" });
  if (config.product_model !== false || config.release_checkpoint !== false) failures.push({ code: "r25ac_config_product_or_release_enabled" });
  if (Number(config.architecture?.layers) !== 1) failures.push({ code: "r25ac_config_must_keep_one_layer_baseline", actual: config.architecture?.layers });
  if (Number(config.language_mix_target?.zh_min) < 0.7 || Number(config.language_mix_target?.en_max) > 0.1) {
    failures.push({ code: "r25ac_language_target_invalid", target: config.language_mix_target });
  }
  if (!datasetReport?.ok) failures.push({ code: "r25ac_dataset_report_missing_or_not_ok" });
  if (!runReport) failures.push({ code: "r25ac_run_report_missing", path: RUN_REPORT_PATH });
  if (runReport?.small_pilot_training_ran === true && !checkpoint) failures.push({ code: "r25ac_replayable_checkpoint_missing", path: CHECKPOINT_PATH });
  if (approval?.consumed !== true) failures.push({ code: "r25ac_approval_not_consumed" });
  if (approval?.allow_additional_runs !== false) failures.push({ code: "r25ac_approval_allows_additional_runs" });
  if (approval?.allow_weight_commit !== false) failures.push({ code: "r25ac_approval_allows_weight_commit" });
  if (approval?.allow_product_model_training !== false) failures.push({ code: "r25ac_approval_allows_product_training" });
  if (approval?.allow_long_term_training !== false) failures.push({ code: "r25ac_approval_allows_long_term_training" });
  if (approval?.allow_phase_4_scaled_training !== false) failures.push({ code: "r25ac_approval_allows_phase_4_scaled_training" });
  if (approval?.allow_release_checkpoint !== false) failures.push({ code: "r25ac_approval_allows_release_checkpoint" });

  if (runReport) {
    if (runReport.skipped === true && runReport.small_pilot_training_ran === false) {
      if (!runReport.reason) failures.push({ code: "r25ac_blocked_without_explicit_reason" });
    } else {
      if (runReport.ok !== true) failures.push({ code: "r25ac_run_report_not_ok" });
      if (runReport.run_id !== "r25ac_chinese_personal_microcycle_256") failures.push({ code: "r25ac_run_id_invalid", actual: runReport.run_id });
      if (runReport.variant_id !== "r25ac_chinese_personal_microcycle_256") failures.push({ code: "r25ac_variant_id_invalid", actual: runReport.variant_id });
      if (runReport.small_pilot_training_ran !== true) failures.push({ code: "r25ac_training_did_not_run_or_block" });
      if (runReport.chinese_personal_microcycle !== true) failures.push({ code: "r25ac_microcycle_flag_missing" });
      if (Number(runReport.actual_layers) !== 1) failures.push({ code: "r25ac_actual_layers_must_be_one", actual: runReport.actual_layers });
      ...
      if (Number(mix.zh || 0) < Number(config.language_mix_target.zh_min)) failures.push({ code: "actual_train_mix_not_zh_primary", mix });
      if (Number(mix.en || 0) > Number(config.language_mix_target.en_max)) failures.push({ code: "actual_train_mix_exceeds_en_cap", mix });
      const coverage = runReport.personal_target_coverage || datasetReport?.personal_target_coverage || {};
      for (const target of config.personal_color_targets || []) {
        if (!coverage[target]) failures.push({ code: "personal_target_missing_from_coverage", target });
        if (coverage[target]?.fabricated !== false) failures.push({ code: "personal_target_coverage_must_not_be_fabricated", target });
      }
      ...
      for (const path of runReport.artifact_paths || []) {
        if (!String(path).startsWith(OUTPUT_DIR)) failures.push({ code: "artifact_outside_r25ac_output_dir", path });
  ...
  const output = {
    ...
    run_id: "r25ac_chinese_personal_microcycle_256",
    variant_id: "r25ac_chinese_personal_microcycle_256",
    ...
    artifacts_under_ignored_path: failures.every((failure) => failure.code !== "artifact_outside_r25ac_output_dir" && failure.code !== "artifact_not_ignored"),
