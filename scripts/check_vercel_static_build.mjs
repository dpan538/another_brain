...
async function main() {
  ...
  const rootHasR28hotfix1DirectApp =
    /R28HOTFIX1|R28HOTFIX2|R28HOTFIX3|R28ROUT1/.test(index) &&
    /过程摘要/.test(index) &&
    /static_q4_experimental/.test(index) &&
    /exact_runtime_tokenizer/.test(index) &&
    /检查本地模型路径/.test(index) &&
    /\/another_brain_chat\/app\.js\?v=(r28hotfix1-route-loop-free-runtime|r28hotfix2-nonblocking-selfcheck|r28hotfix3-q4-asset-path-fix|r28rout1-fuzzy-intent-surfaces)/.test(index);
