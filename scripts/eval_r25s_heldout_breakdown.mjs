...
const OUTPUT_PATH = "artifacts/training_os/small_decoder_pilot/r25t/r25t_r25s_heldout_breakdown.json";
const CHECKPOINT_PATH = "artifacts/training_os/small_decoder_pilot/r25s/r25s_replayable_checkpoint.json";
const HELDOUT_SEQUENCES_PATH = "artifacts/training_os/small_decoder_pilot/r25s/r25s_heldout_sequences.json";
...
async function main() {
  ...
  if (missing.length) {
    const report = {
      ...
      missing,
      training_ran: false,
      notes: ["R25S heldout breakdown is skipped because local ignored replay artifacts are absent."]
    };
  ...
  const report = {
    ...
    notes: [
      "Breakdown uses R25L heldout rows and the existing ignored R25S checkpoint only.",
      "No training is performed.",
      "Average losses are weighted by next-token pair count within each group."
    ],
    failures
  };
  const previous = await readJsonIfPresent(R25Q_BREAKDOWN_PATH);
  report.weak_bucket_deltas_vs_r25p = weakBucketDeltas(report, previous);
  await writeJson(OUTPUT_PATH, report);
  console.log(JSON.stringify(report, null, 2));
