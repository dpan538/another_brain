...
const out = join(tmpdir(), `r28p0c-ts-tests-${process.pid}`);
...
await copyTsAsMjs(join(root, "tests/r28p0c"), join(out, "tests/r28p0c"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await cp(join(root, "scripts/r28p0c_coldstart_matrix.mjs"), join(out, "scripts/r28p0c_coldstart_matrix.mjs"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28p0c/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
