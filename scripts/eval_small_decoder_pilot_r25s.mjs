...
const APPROVAL_PATH = "training/from_scratch/APPROVE_R25S_DATA_FIRST_PILOT.json";
const OUTPUT_DIR = "artifacts/training_os/small_decoder_pilot/r25s/";
const RUN_REPORT_PATH = `${OUTPUT_DIR}r25s_small_decoder_run_report.json`;
const DATASET_REPORT_PATH = `${OUTPUT_DIR}r25s_dataset_report.json`;
const CHECKPOINT_PATH = `${OUTPUT_DIR}r25s_replayable_checkpoint.json`;
const EVAL_REPORT_PATH = `${OUTPUT_DIR}r25s_small_decoder_eval_report.json`;
...
function validateReplayableCheckpoint(checkpoint) {
  ...
  if (checkpoint.run_id !== "r25s_data_first_balanced_192") failures.push({ code: "checkpoint_run_id_invalid", actual: checkpoint.run_id });
  if (checkpoint.phase !== "phase_3_small_decoder_pilot") failures.push({ code: "checkpoint_phase_invalid", actual: checkpoint.phase });
  if (!["decoder_like_next_token_pilot", "causal_decoder_pilot", "decoder_only_transformer_pilot"].includes(checkpoint.model_type)) {
...
async function main() {
  ...
  if (config.run_id !== "r25s_data_first_balanced_192" || config.variant_id !== "r25s_data_first_balanced_192") {
    failures.push({ code: "r25s_config_variant_mismatch", run_id: config.run_id, variant_id: config.variant_id });
  }
  if (config.phase_4_scaled_training !== false) failures.push({ code: "r25s_config_phase_4_must_be_false" });
  if (!datasetReport?.ok) failures.push({ code: "r25s_dataset_report_missing_or_not_ok" });
  if (!runReport) failures.push({ code: "r25s_run_report_missing", path: RUN_REPORT_PATH });
  if (!checkpoint) failures.push({ code: "r25s_replayable_checkpoint_missing", path: CHECKPOINT_PATH });
  if (approval?.consumed !== true) failures.push({ code: "r25s_approval_not_consumed" });
  if (approval?.allow_weight_commit !== false) failures.push({ code: "r25s_approval_allows_weight_commit" });
  if (approval?.allow_product_model_training !== false) failures.push({ code: "r25s_approval_allows_product_training" });
  if (approval?.allow_long_term_training !== false) failures.push({ code: "r25s_approval_allows_long_term_training" });
  if (approval?.allow_phase_4_scaled_training !== false) failures.push({ code: "r25s_approval_allows_phase_4_scaled_training" });
  if (approval?.allow_release_checkpoint !== false) failures.push({ code: "r25s_approval_allows_release_checkpoint" });

  if (runReport) {
    if (runReport.ok !== true) failures.push({ code: "r25s_run_report_not_ok" });
    if (runReport.run_id !== "r25s_data_first_balanced_192") failures.push({ code: "r25s_run_id_invalid", actual: runReport.run_id });
    if (runReport.variant_id !== "r25s_data_first_balanced_192") failures.push({ code: "r25s_variant_id_invalid", actual: runReport.variant_id });
    if (runReport.small_pilot_training_ran !== true) failures.push({ code: "r25s_training_did_not_run" });
    ...
    for (const path of runReport.artifact_paths || []) {
      if (!String(path).startsWith(OUTPUT_DIR)) failures.push({ code: "artifact_outside_r25s_output_dir", path });
  ...
  if (datasetReport?.balanced_sampling_used !== true) failures.push({ code: "r25s_dataset_did_not_use_balanced_sampling_plan" });
  ...
  const output = {
    ...
    artifacts_under_ignored_path: failures.every((failure) => failure.code !== "artifact_outside_r25s_output_dir" && failure.code !== "artifact_not_ignored"),
