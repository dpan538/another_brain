...
const out = join(tmpdir(), `r28rag3-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28rag3"), join(out, "tests/r28rag3"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });

const result = spawnSync("node", ["--test", join(out, "tests/r28rag3/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
