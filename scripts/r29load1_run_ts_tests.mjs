...
const out = join(tmpdir(), `r29load1-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r29load1"), join(out, "tests/r29load1"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r29load1/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
