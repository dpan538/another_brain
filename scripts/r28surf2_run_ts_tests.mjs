...
const out = join(tmpdir(), `r28surf2-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28surf2"), join(out, "tests/r28surf2"));
await cp(join(root, "web"), join(out, "web"), { recursive: true });
await cp(join(root, "data/training_registry"), join(out, "data/training_registry"), { recursive: true });
await cp(join(root, "vercel.json"), join(out, "vercel.json"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28surf2/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
