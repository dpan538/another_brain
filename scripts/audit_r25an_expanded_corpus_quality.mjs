...
const REPORT_PATH = "artifacts/training_os/corpus_review/r25an/r25an_expanded_corpus_quality.json";
const SUMMARY_PATH = "docs/R25AN_EXPANDED_CORPUS_QUALITY_SUMMARY.md";
...
function share(count, total) {
  return total ? Math.round((count / total) * 10000) / 10000 : 0;
}
...
async function main() {
  ...
  for (const row of rows) {
    ...
    if (target) {
      ...
      targetMap.set(rawHash, (targetMap.get(rawHash) || 0) + 1);
      normalizedTargetMap.set(normalizedHash, (normalizedTargetMap.get(normalizedHash) || 0) + 1);
    }
    ...
    for (const ref of sourceFileRefs(row)) {
      if (FORBIDDEN_SOURCE_RE.test(ref)) {
        forbiddenSourceRefs.push({ file: row.__file, line: row.__line, source_ref_hash: createHash("sha256").update(ref).digest("hex") });
      }
    }
    for (const marker of collectForbiddenKeys(row)) privateMarkers.push({ file: row.__file, line: row.__line, marker: marker.key, path: marker.path });
  }
  ...
  const r25amRows = rows.filter((row) => String(row.__file).startsWith("r25am_repo_derived_"));
  const r25akLanguageCounts = {};
  const r25amLanguageCounts = {};
  for (const row of r25akRows) countKey(r25akLanguageCounts, row.language);
  for (const row of r25amRows) countKey(r25amLanguageCounts, row.language);
  const zhShare = share(languageCounts.zh || 0, rows.length);
  const mixedShare = share(languageCounts.mixed || 0, rows.length);
  const enShare = share(languageCounts.en || 0, rows.length);
  const warnings = [];
  if (zhShare < 0.7) warnings.push("combined_corpus_below_future_zh_70_target");
  if (enShare > 0.1) warnings.push("combined_corpus_above_future_en_10_target");
  ...
  const report = {
    ...
    language_shares: { zh: zhShare, mixed: mixedShare, en: enShare },
    language_counts_by_file: languageByFile,
    r25ak_contribution: { rows: r25akRows.length, language_counts: r25akLanguageCounts },
    r25am_contribution: { rows: r25amRows.length, language_counts: r25amLanguageCounts },
    ...
    chinese_first_gap: {
      ...
      target_mixed: 0.2,
      target_en_max: 0.1,
      combined_zh_share: zhShare,
      combined_mixed_share: mixedShare,
      combined_en_share: enShare,
      remaining_zh_rows_needed_at_current_total: Math.max(0, Math.ceil(0.7 * rows.length - (languageCounts.zh || 0))),
      en_rows_above_10_percent_at_current_total: Math.max(0, (languageCounts.en || 0) - Math.floor(0.1 * rows.length)),
      r25am_subset_zh_share: share(r25amLanguageCounts.zh || 0, r25amRows.length)
    },
    ...
    safety: {
      ...
      evals_used_as_training_data: false,
  ...
  const summary = `# R25AN Expanded Corpus Quality Summary
...
- R25AM contribution: ${r25amRows.length} rows; zh ${r25amLanguageCounts.zh || 0}, mixed ${r25amLanguageCounts.mixed || 0}, en ${r25amLanguageCounts.en || 0}
...
R25AM moved the combined corpus toward Chinese-first training: zh share is ${Math.round(zhShare * 10000) / 100}%, mixed share is ${Math.round(mixedShare * 10000) / 100}%, and en share is ${Math.round(enShare * 10000) / 100}%. The full corpus remains below the future zh >= 70% target under uniform sampling, so any later micro-cycle needs a fresh approval and a zh-first sampler or more reviewed Chinese rows.
`;
