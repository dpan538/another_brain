...
const PILOT_ARTIFACT_RE = /r25m_|r25n_|r25o_|r25p_|r25s_|r25v_|r25y_|small_decoder_checkpoint|small_decoder_metrics|small_decoder_run_report|small_decoder_pilot/i;
...
async function main() {
  ...
  for (const root of forbiddenRoots) {
    for (const path of await walk(root)) {
      if (PILOT_ARTIFACT_RE.test(path) && /r25m|r25n|r25o|r25p|r25s|r25v|r25y|checkpoint|metrics|run_report|train_sequences|dev_sequences|heldout_sequences/.test(path)) {
        misplaced.push(path);
      }
    }
  }
