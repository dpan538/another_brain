...
def main():
    ...
    out = ROOT / "artifacts/r27a5/curriculum/value_aesthetic.jsonl"; write_jsonl(out, rows)
    ...
    (ROOT / "artifacts/r27a5/reports/value_aesthetic_report.json").write_text(json.dumps(report, indent=2), encoding="utf-8")
    (ROOT / "docs/r27/R27A5_VALUE_RAG_REASONING_EXPANSION.md").write_text(f"# R27A5 Value RAG Reasoning Expansion\n\nValue/aesthetic rows built: `{len(rows)}`. Rows derive from approved R26E/R26G user-answer anchors, preserve source ids, and exclude rows 9, 16, and old question_pack_001 rows 51-100.\n", encoding="utf-8")
