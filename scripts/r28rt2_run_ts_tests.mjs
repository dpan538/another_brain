...
const out = join(tmpdir(), `r28rt2-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28rt2"), join(out, "tests/r28rt2"));
await cp(join(root, "package.json"), join(out, "package.json"));

const result = spawnSync("node", ["--test", join(out, "tests/r28rt2/*.mjs")], {
  cwd: root,
  shell: true,
  stdio: "inherit"
});
