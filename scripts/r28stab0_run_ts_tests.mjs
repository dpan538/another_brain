...
const out = join(tmpdir(), `r28stab0-ts-tests-${process.pid}`);
...
await copyAsMjs(join(root, "tests/r28stab0"), join(out, "tests/r28stab0"));
...
const result = spawnSync("node", ["--test", join(out, "tests/r28stab0/*.mjs")], {
  cwd: out,
  shell: true,
  stdio: "inherit"
});
