...
R26D uses the answered CSV only from the ignored project-local path `private_sources/question_packs/another_brain_question_pack_001_answered.csv`. It parses rows 1-50 as answer-as-user candidates and writes candidate rows only under ignored artifacts. It does not promote rows into `training/llm_corpus`, run tokenizer dry-run, train, or commit the raw CSV.

Rows 51-100 remain excluded during R26D and cannot be used for candidates, preference pairs, repair pairs, teacher probes, tokenizer text, eval-derived training seeds, long-horizon rows, prompt generation, corpus generation, or corpus promotion.
