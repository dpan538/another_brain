...
R16 trains a controlled gate only: a multi-head classifier over labels such as domain, task type, question type, operation, answer policy, risk label, verifier label, and template id.
...
This is controlled training, not mini-LLM generative training.

## Export Policy

`web/controlled_gate_model.generated.js` may be written only if:

- blind metrics meet the R16 thresholds;
...
The first R16 version is a dry-run classifier scaffold. It is useful for measuring label coverage and blind failures, but it should not be integrated into runtime until metrics and browser budget are strong enough.
