...
test("P0C hotfix cache-busts app, runtime worker, self-check worker, and q4 runtime modules", async () => {
...
test("runtime mode metadata invalidates the old SHIP0/P0 asset cache namespace", async () => {
  ...
  assert.ok(app.includes("url.searchParams.set(\"v\", R28P0C_COLDSTART_MOBILE_CHAT_VERSION)"));
  ...
  assert.equal(assetManifest.ui_version, "r28p0c-coldstart-mobile-chat");
