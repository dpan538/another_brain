...
test("P0B hotfix cache-busts app, runtime worker, self-check worker, and q4 runtime modules", async () => {
  ...
  for (const source of [index, app, runtime, runtimeWorker, selfCheckWorker]) {
    assert.ok(source.includes("r28p0b-primary-q4-mount-state"));
  }
...
test("runtime mode metadata invalidates the old SHIP0/P0 asset cache namespace", async () => {
  ...
  assert.ok(app.includes("url.searchParams.set(\"v\", R28P0B_PRIMARY_Q4_MOUNT_STATE_VERSION)"));
  assert.ok(app.includes("fetch(url.href, { cache: \"no-store\" })"));
  assert.equal(runtimeMode.ui_version, "r28p0b-primary-q4-mount-state");
  assert.equal(runtimeMode.asset_cache_version, "r28p0b-primary-q4-mount-state");
  assert.equal(assetManifest.ui_version, "r28p0b-primary-q4-mount-state");
