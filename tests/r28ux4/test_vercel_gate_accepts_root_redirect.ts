...
test("vercel static gate accepts the R28UX4 root chat redirect shape", async () => {
  ...
  const acceptedVersions = ["r28ux4-visible-preview-ui", "r28hotfix0-runtime-ui-activation"];
  ...
  assert.ok(acceptedVersions.some((version) => html.includes(`another_brain_chat/?v=${version}`)));
});
