...
test("the shipped relay satisfies its own contract", async () => {
  ...
  for (const p of ["api/chat.js", "app/server/chat_handler.js", "app/server/dev_middleware.js"]) {
...
test("a key literal, a client-visible key, a log line or another host are each refused", () => {
  ...
  assert.ok(codes(serverProxyFileFailures("api/chat.js", 'fetch("https://api.openai.com/v1/chat")')).includes("server_proxy_references_other_model_host"));
});
...
test("a relay that lets the caller pick the model, or skips the origin check, is refused", async () => {
  ...
  const broken = (edit) => { const m = new Map([["app/server/chat_handler.js", edit(good)], ["api/chat.js", "export default (r) => handleChat(r, {})"]]); return codes(serverProxyContractFailures(m)); };
...
test("a second route, or a bundle that reads the server environment, is refused", async () => {
  ...
  const m = new Map([["app/server/chat_handler.js", handler], ["api/chat.js", "export default (r) => handleChat(r, {})"], ["api/other.js", "export default () => new Response('x')"], ["app/src/x.js", "const k = process.env.DEEPSEEK_API_KEY"]]);
