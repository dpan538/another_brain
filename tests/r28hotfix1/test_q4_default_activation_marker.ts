...
test("q4 runtime activation markers are the default when assets are available", async () => {
  ...
  assert.equal(runtimeMode.ui_version, "r28hotfix2-nonblocking-selfcheck");
  assert.equal(manifest.ui_version, "r28hotfix2-nonblocking-selfcheck");
  assert.ok(runtime.includes("R28HOTFIX2 q4 path smoke"));
