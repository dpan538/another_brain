...
test("q4 runtime activation markers are the default when assets are available", async () => {
  ...
  assert.equal(runtimeMode.ui_version, "r28hotfix1-route-loop-free-runtime");
  assert.equal(manifest.ui_version, "r28hotfix1-route-loop-free-runtime");
  assert.ok(runtime.includes("R28HOTFIX1 q4 path smoke"));
