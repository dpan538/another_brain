...
test("R28HOTFIX1 keeps non-product claims visible", async () => {
  ...
  assert.equal(manifest.external_llm_api, false);
  assert.equal(manifest.doubao, false);
  assert.equal(manifest.hosted_vector_store, false);
});
