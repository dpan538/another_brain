...
test("query routes have direct static entries instead of canonicalization scripts", async () => {
  ...
  assert.ok(noSlash.includes("R28HOTFIX1") || noSlash.includes("R28HOTFIX2") || noSlash.includes("R28HOTFIX3"));
  assert.ok(slash.includes("R28HOTFIX1") || slash.includes("R28HOTFIX2") || slash.includes("R28HOTFIX3"));
