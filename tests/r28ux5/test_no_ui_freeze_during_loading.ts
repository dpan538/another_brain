...
test("loading and self-check stay abortable and progress-driven while chat requires full q4 mount", async () => {
  ...
  assert.ok(app.includes("modelFullyLoaded"));
  assert.ok(app.includes("q4_mount_required_before_chat"));
  assert.doesNotMatch(app, /loadingCancelButton/);
