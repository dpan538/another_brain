...
def run(*, prefer_device: str = "mps", resource_safe: bool = True, config: RunConfig = CONFIG) -> dict[str, Any]:
    ...
    for index in range(1, config.policy["max_segments"] + 1):
        ...
        segment = {"segment_index": index, "optimizer_tokens": tokens, "optimizer_steps": steps, "running_train_loss": acc.to_report(), "dev_loss": dev_eval.get("average_loss"), "heldout_loss": heldout_eval.get("average_loss"), "probe": probe, "checkpoint_path": str(checkpoint.relative_to(ROOT)), "control_contract": contract}; ledger["segments"].append(segment); ledger["optimizer_tokens"] += tokens; ledger["optimizer_steps"] += steps; write_json(paths["runs"] / config.campaign_id / f"segment_{index:02d}.json", segment); write_json(paths["ledger"], ledger); _heartbeat(paths, config, contract, "segment_evaluated", segment_index=index, optimizer_tokens=ledger["optimizer_tokens"], optimizer_steps=ledger["optimizer_steps"], probe_average_score=probe["probe_average_score"], heldout_loss=segment["heldout_loss"])
        if probe["below_threshold"] or probe["role_prefix_leaks"]:
            ledger["stop_reason"] = "probe_quality_below_threshold"; write_json(paths["ledger"], ledger); break
        if index > 1 and probe["probe_average_score"] < best_probe - .05:
            ledger["stop_reason"] = "probe_regression_stop"; write_json(paths["ledger"], ledger); break
