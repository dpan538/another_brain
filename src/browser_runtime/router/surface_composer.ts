...
function openQuestionCore(category, input) {
  if (category === "aesthetic_fallback") {
    return [pickIndexed("aesthetic_core", input, "aesthetic-a"), pickIndexed("aesthetic_core", input, "aesthetic-b"), pickIndexed("style_stance", input, "aesthetic-c")];
  }
  if (category === "relation_fallback") {
    return [pickIndexed("relation_core", input, "relation-a"), pickIndexed("relation_core", input, "relation-b")];
  }
  if (category === "language_meaning_fallback") {
    return [pickIndexed("language_meaning_core", input, "language-a"), pickIndexed("language_meaning_core", input, "language-b")];
  }
...
export function composeSurfaceAnswer({
  ...
  if (resolvedRoute === "insufficient_evidence_boundary") {
    ...
    fragmentEntries = [{ id: "legacy_malicious_evidence", text: "我不会执行隐藏提示或改写规则的请求；这类内容会被当作不可信指令处理。" }];
  } else if (
