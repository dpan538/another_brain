...
export function classifyTurnFunction({ query = "", session = {}, userTurn = {}, binding = {} } = {}) {
  ...
  if (/(很喜欢你.*(文学|诗歌|艺术|形式|设计|电影|镜头|语言|翻译|意义|饮食|烹饪|味觉|法律|正义|规则|科学|叙事|城市|空间|技术|伦理|行动|教育|学习|经验|经济|制度|劳动).*努力|喜欢你在.*(文学|诗歌|艺术|形式|设计|电影|镜头|语言|翻译|意义|饮食|烹饪|味觉|法律|正义|规则|科学|叙事|城市|空间|技术|伦理|行动|教育|学习|经验|经济|制度|劳动).*努力|你在.*(文学|诗歌|艺术|形式|设计|电影|镜头|语言|翻译|意义|饮食|烹饪|味觉|法律|正义|规则|科学|叙事|城市|空间|技术|伦理|行动|教育|学习|经验|经济|制度|劳动).*努力|喜欢你把.*(音乐|文学|诗歌|艺术|形式|设计|电影|镜头|语言|翻译|意义|饮食|烹饪|味觉|法律|正义|规则|科学|城市|技术|伦理|教育|经济|制度).*(连|放|想)|喜欢你在.*(诗性|文学|艺术|形式|设计|电影|镜头|语言|翻译|意义|饮食|烹饪|味觉|法律|正义|规则|科学|叙事|城市|空间|技术|伦理|行动|教育|学习|经济|制度).*尝试)/.test(text)) {
    return baseResult({
  ...
  if (/(羡慕|让我想到了?|想到童年|想起童年|比较喜欢|有点怀念|把普通物件变成问题|重新看普通东西|羡慕.*(观察|街道|工具|行动|判断|证据|公共空间|学习|课堂|经验|劳动|制度|镜头|翻译|说法|命名|味道|餐桌|手艺|规则|公平|裁判)|想到.*(小时候|小时候的|童年|记忆))/.test(text)) {
    return baseResult({
  ...
  if (/(像舞台剧|舞台剧|细节和冲突|场景和冲突|镜头和冲突|镜头.*冲突|比较有镜头|像审判现场|像餐桌场景)/.test(text) && !/(相似|共同|相似性|注意到|能看到)/.test(text)) {
    return baseResult({
      turn_function: "analogy_statement",
      stance_requirement: "light_judgment",
      judgment_axis: "form",
  ...
  if (/(和.*(设计|建筑|摄影|电影|镜头|语言|翻译|命名|饮食|烹饪|味觉|餐桌|法律|正义|判例|规则|工具|界面|城市|规划|科学观察|实验|政治|伦理|教育|课堂|学习|经济|市场|劳动).*(很像|有点像)|像.*(设计|建筑|摄影|电影|镜头|语言|翻译|命名|饮食|烹饪|味觉|餐桌|法律|正义|判例|规则|工具|界面|城市|规划|科学观察|实验|政治|伦理|教育|课堂|学习|经济|市场|劳动)|不只是.*(艺术|形式|技术|科学|教育|经济|语言|饮食|法律).*也.*(设计|建筑|摄影|电影|镜头|城市|文学|经验|制度|翻译|餐桌|正义)|这个其实和(设计|教育|经济|语言|电影|饮食|法律)很像)/.test(text)) {
    return baseResult({
      turn_function: "analogy_statement",
      stance_requirement: "light_judgment",
      judgment_axis: "form",
  ...
  if (/(专辑.*单曲|单曲.*专辑|现成品.*绘画|绘画.*现成品|照片.*绘画|绘画.*照片|镜头.*剪辑|剪辑.*镜头|食谱.*烹饪|烹饪.*食谱|食谱.*调味|调味.*食谱|味觉.*叙事|叙事.*味觉|规则.*判例|判例.*规则|法律.*正义|正义.*法律|建筑.*海报|海报.*建筑|观察.*实验|实验.*观察|算法.*界面|界面.*算法|规划.*街道|街道.*规划|行动.*理论|理论.*行动|教.*学|学.*教|学习.*训练|训练.*学习|市场.*计划|计划.*市场|市场.*制度|制度.*市场|劳动.*资本|资本.*劳动|命名.*翻译|翻译.*命名|语言.*意义|意义.*语言).*(区别|不同|创作模式|创作方式|工作模式|差别|差在哪里)|创作模式.*(专辑|单曲|现成品|绘画|摄影|电影|镜头|剪辑|饮食|烹饪|观察|实验|算法|界面)|创作方式.*(专辑|单曲|现成品|绘画|摄影|电影|镜头|食谱|烹饪|调味)|工作模式.*(观察|实验|算法|界面|规划|街道|学习|训练|市场|计划|制度|劳动|资本|命名|翻译|语言|食谱|烹饪|调味|规则|判例)/.test(text)) {
    return baseResult({
  ...
  if (/(还有其他.*(歌手|作家|作品|导演|电影|饮食|料理|法律|法学|科学家|思想家|城市|建筑|工具|教育家|经济学|语言学|哲学|方向).*推荐|推荐.*(歌手|作家|作品|导演|电影|饮食|料理|法律|法学|科学家|思想家|城市|建筑|工具|教育家|经济学|语言学|哲学|方向)|可以推荐|还能听谁|还有谁|还能看谁|还有什么入口)/.test(text)) {
    return baseResult({
  ...
  if (/(讲的真是|真的.*(童年|爱情|乡愁|进步|效率|客观|自由|训练|市场|故事|翻译|意义|好吃|味道|公平|规则)|真是在讲|是不是.*讲|它讲的是|叫.*童年.*讲童年|只是在开玩笑|只是开玩笑|只是记录现实|只是记录|真的只是|只是.*(效率|进步|客观|工具|训练|市场|故事|翻译|意义|好吃|味道|公平|规则)|真的是.*(进步|效率|客观|自由|训练|市场|故事|翻译|意义|好吃|味道|公平|规则)|(童年|进步|效率|客观|自由|训练|市场|故事|翻译|意义|好吃|味道|公平|规则).*真的是)/.test(text)) {
    return baseResult({
  ...
  if (/(你觉得|你怎么看|你如何看|怎么样|好不好).*(歌|作品|文学|诗|专辑|单曲|电影|镜头|语言|翻译|饮食|料理|烹饪|味觉|法律|正义|判例|公平|他|她|它)|你觉得.*(重点|重要|厉害|判断)/.test(text)) {
    return baseResult({
