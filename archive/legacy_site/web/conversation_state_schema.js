...
export function makeConversationStatePatch({
  lastUserQuery = "",
  lastTurnFunction = "",
  lastStanceRequirement = "",
  lastJudgmentAxis = "",
  lastAffectiveLoad = "",
  lastIdentityBoundaryLevel = "",
  lastBridgeTarget = "",
  lastAssistantAnswer = "",
  lastAnswerSource = "",
  lastAnswerQuality = "",
  ...
  return {
    ...
    lastTurnFunction: clean(lastTurnFunction),
    last_turn_function: clean(lastTurnFunction),
    lastStanceRequirement: clean(lastStanceRequirement),
    last_stance_requirement: clean(lastStanceRequirement),
    lastJudgmentAxis: clean(lastJudgmentAxis),
    last_judgment_axis: clean(lastJudgmentAxis),
    lastAffectiveLoad: clean(lastAffectiveLoad),
    last_affective_load: clean(lastAffectiveLoad),
    lastIdentityBoundaryLevel: clean(lastIdentityBoundaryLevel),
    last_identity_boundary_level: clean(lastIdentityBoundaryLevel),
    lastBridgeTarget: clean(lastBridgeTarget),
    last_bridge_target: clean(lastBridgeTarget),
...
export function buildConversationStatePatch({ query = "", answer = "", resolved = {}, finalized = {}, previousState = {} } = {}) {
  ...
  return makeConversationStatePatch({
    lastUserQuery: query,
    lastTurnFunction: resolved.turnFunction?.turn_function || trace.turn_function || "",
    lastStanceRequirement: resolved.turnFunction?.stance_requirement || trace.stance_requirement || "",
    lastJudgmentAxis: resolved.turnFunction?.judgment_axis || trace.judgment_axis || "",
    lastAffectiveLoad: resolved.turnFunction?.affective_load || trace.affective_load || "",
    lastIdentityBoundaryLevel: resolved.turnFunction?.identity_boundary_level || trace.identity_boundary_level || "",
    lastBridgeTarget: resolved.turnFunction?.bridge_target || trace.bridge_target || "",
    lastAssistantAnswer: answer,
