// Compiles a LocalSignalPacket into an advisory system message.
//
// Ported from src/hybrid_runtime/style_policy_compiler.ts. Used only when the
// local-signal seam is explicitly enabled; see local_signal_provider.js for the
// evidence on why it is off by default.

import { assertPacketGrounded } from "./local_signal_provider.js";
...
export function compileStylePolicy(packet, groundingText, omittedFields = new Set()) {
  assertPacketGrounded(packet, groundingText);
